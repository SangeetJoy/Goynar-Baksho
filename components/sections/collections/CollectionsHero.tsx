import React from "react";

export const CollectionsHero: React.FC = () => (
  <section className="py-16 lg:py-20 bg-brand-bg">
    <div className="container mx-auto px-4 text-center">
      <h1 className="text-5xl lg:text-6xl font-serif mb-4 uppercase tracking-wide text-brand-heading">
        Our Collections
      </h1>
      <p className="text-lg lg:text-xl max-w-2xl mx-auto text-white opacity-80">
        Discover our exquisite collection of gold-plated jewelry, crafted with
        precision and designed to make you shine.
      </p>
    </div>
  </section>
);
